...
echo "Checking VPS"
#########################
IZIN=$(curl -sS https://raw.githubusercontent.com/SLSTunnel/izin/master/sshws | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
#EXPIRED
expired=$(curl -sS https://raw.githubusercontent.com/SLSTunnel/izin/master/sshws | grep $MYIP | awk '{print $3}')
echo $expired > /root/expired.txt
today=$(date -d +1day +%Y-%m-%d)
while read expired
do
	exp=$(echo $expired | curl -sS https://raw.githubusercontent.com/SLSTunnel/izin/master/sshws | grep $MYIP | awk '{print $3}')
	if [[ $exp < $today ]]; then
		Exp2="\033[1;31mExpired\033[0m"
        else
        Exp2=$(curl -sS https://raw.githubusercontent.com/SLSTunnel/izin/master/sshws | grep $MYIP | awk '{print $3}')
	fi
done < /root/expired.txt
rm /root/expired.txt
Name=$(curl -sS https://raw.githubusercontent.com/SLSTunnel/izin/master/sshws | grep $MYIP | awk '{print $2}')
# Color Validation
DF='\e[39m'
Bold='\e[1m'
Blink='\e[5m'
